...
import type { LeadResult } from "@/lib/schemas";
...
export function CrmOutput({ leadId, result }: CrmOutputProps) {
  ...
  const json = JSON.stringify(toCrmOutput(leadId, result), null, 2);
  ...
  return (
    <section className="min-w-0 overflow-hidden rounded-xl border border-[#dfe3e9] bg-white shadow-[0_1px_2px_rgba(20,26,40,0.04)]">
      ...
      <pre className="overflow-x-auto bg-[#1c2430] px-5 py-4 font-mono text-xs leading-6 text-[#dce5f2]">
        <code>{json}</code>
      </pre>
